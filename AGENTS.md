...
- Avoid using `useEffect` for values that can be calculated during render.
...
- TanStack Router:
  ...
  - Use route loaders for route-level data requirements
  - Keep route definitions focused
  - Use route context for shared dependencies where appropriate
  ...
  - Prefer route-level data loading when data is required before rendering a route instead of fetching it from a client-side `useEffect`
